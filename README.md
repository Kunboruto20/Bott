...
try {
  ...
  if (error.name === 'IgLoginTwoFactorRequiredError') {
    ...
    await ig.account.twoFactorLogin({
      ...
      twoFactorIdentifier: error.response.data.twofactorinfo.twofactoridentifier
    });
  }
}
`
...
for (const item of thread.thread.items) {
  await ig.directThread.markItemSeen(thread.thread.threadid, item.itemid);
}
`
...
// Delete comment
await ig.media.deleteComment('mediaid', 'commentid');
`
